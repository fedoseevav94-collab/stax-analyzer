...
```text
ID: {conversation_id}
#1 [Клиент episode=1 date={date} id={message_id}]: текст сообщения
#2 [Сотрудник episode=1 date={date} id={message_id}]: текст сообщения
```

Если у сообщения нет `created_date`, `date` или `timestamp`, дата в строке не добавляется.
Если у сообщения нет `id`, id в строке не добавляется.
