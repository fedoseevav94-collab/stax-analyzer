...
4. Сначала сделай поверхностное понимание: кто что спросил, что ответил сотрудник, чем закончился эпизод.
5. Потом проверь только явные расхождения/эскалации внутри этого же эпизода.
6. Не используй сообщение клиента, написанное позже ответа сотрудника, как основание для проблемы в этом ответе.
7. Проблемы ставь только за сообщения сотрудника.
8. Сообщения клиента используй только как контекст или доказательство жалобы.
9. Если роль автора неясна — не ставь проблему.
10. Не используй внешние знания, бизнес-логику или предположения.
11. Нужна точная цитата сотрудника, где нарушение видно напрямую.
12. Если confidence ниже 0.8 или есть сомнение — не добавляй проблему.
13. Если диалог нормальный, короткий, бытовой или неполный — верни отсутствие проблем.
...
- спокойно сообщает отказ от сотрудничества или невозможность продолжить сотрудничество;
- пытается разобраться.

В STAX сотрудник может не раскрывать подробную внутреннюю причину отказа водителю/клиенту. Не считай это конфликтом или некомпетентностью, если сотрудник спокойно сообщил отказ без агрессии.
